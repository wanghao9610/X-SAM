# Copied and adapted from https://github.com/facebookresearch/Mask2Former/blob/main/datasets/prepare_ade20k_pan_seg.py
import glob
import json
import os
...
if __name__ == "__main__":
    dataset_dir = os.getenv("root_dir", ".") / "datas"

    for name, dirname in [("train", "training"), ("val", "validation")]:
        image_dir = osp.join(dataset_dir, f"ade20k/images/{dirname}/")
        semantic_dir = osp.join(dataset_dir, f"ade20k/annotations/{dirname}/")
        instance_dir = osp.join(dataset_dir, f"ade20k/annotations_instance/{dirname}/")

        # folder to store panoptic PNGs
        out_folder = osp.join(dataset_dir, f"ade20k/ade20k_panoptic_{name}/")
        # json with segmentations information
        out_file = osp.join(dataset_dir, f"ade20k/ade20k_panoptic_{name}.json")
        ...
        # json config
        config_file = osp.join(this_dir, "ade20k_instance_imgCatIds.json")
        with open(config_file) as f:
            config = json.load(f)

        # load catid mapping
        mapping_file = osp.join(this_dir, "ade20k_instance_catid_mapping.txt")
        ...
        filenames = sorted(glob.glob(osp.join(image_dir, "*.jpg")))
        for idx, filename in enumerate(tqdm.tqdm(filenames)):
            ...
            pan_seg = np.zeros((original_format.shape[0], original_format.shape[1], 3), dtype=np.uint8)
            id_generator = IdGenerator(categories_dict)

            filename_semantic = osp.join(semantic_dir, image_id + ".png")
            filename_instance = osp.join(instance_dir, image_id + ".png")
            ...
            Image.fromarray(pan_seg).save(osp.join(out_folder, image_id + ".png"))
