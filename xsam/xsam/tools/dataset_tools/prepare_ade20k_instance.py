# Copied and adapted from https://github.com/facebookresearch/Mask2Former/blob/main/datasets/prepare_ade20k_ins_seg.py
import glob
import json
import os
...
if __name__ == "__main__":
    ...
    for name, dirname in [("train", "training"), ("val", "validation")]:
        ...
        # json
        out_file = osp.join(dataset_dir, f"ade20k/ade20k_instance_{name}.json")

        # json config
        instance_config_file = osp.join(this_dir, "datasets/ade20k_instance_imgCatIds.json")
        ...
        mapping_file = osp.join(this_dir, "datasets/ade20k_instance_catid_mapping.txt")
        ...
        for idx, filename in enumerate(tqdm.tqdm(filenames)):
            image = {}
            image_id = osp.basename(filename).split(".")[0]

            image["id"] = image_id
            image["file_name"] = osp.basename(filename)
