...
def parse_args():
    parser = argparse.ArgumentParser(description="Convert the pth model to HuggingFace model")
    parser.add_argument("config", help="config file name or path.")
    parser.add_argument("work_dir", help="the dir to save logs and models")
    parser.add_argument("--pth_model", default="latest", help="pth model file")
    parser.add_argument(
        "--fp32",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=str,
        default="2GB",
        help="Only applicable for LLM. The maximum size for " "each sharded checkpoint.",
    )
    parser.add_argument(
        "--safe-serialization",
        action="store_true",
...
def main():
    ...
    if hasattr(cfg.model, "llm") and hasattr(cfg.model.llm, "quantization_config"):
