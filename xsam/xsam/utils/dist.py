...
def init_sequence_parallel(sequence_parallel_size: int = 1):
    ...
    if world_size % sequence_parallel_size != 0:
        raise RuntimeError(
            f"world_size ({world_size}) is not divisible by " f"sequence_parallel_size {sequence_parallel_size}"
        )
...
def init_inner_sequence_parallel(inner_sequence_parallel_size: int = 1):
    ...
    assert _SEQUENCE_PARALLEL_GROUP is not None, (
        "Please call `init_inner_sequence_parallel` after calling " "`init_sequence_parallel`."
    )
    ...
    for i in range(n_inner_group):
        ranks = range(i * inner_sequence_parallel_size, (i + 1) * inner_sequence_parallel_size)
        group = dist.new_group(ranks)
        if rank in ranks:
...
def get_inner_sequence_parallel_world_size():
    ...
    if not dist.is_initialized() or (_INNER_SEQUENCE_PARALLEL_GROUP is None):
        ...
        _INNER_SEQUENCE_PARALLEL_WORLD_SIZE = dist.get_world_size(group=get_inner_sequence_parallel_group())
    return _INNER_SEQUENCE_PARALLEL_WORLD_SIZE
...
def get_inner_sequence_parallel_rank():
    ...
    if not dist.is_initialized() or (_INNER_SEQUENCE_PARALLEL_GROUP is None):
        ...
        _INNER_SEQUENCE_PARALLEL_RANK = dist.get_rank(group=get_inner_sequence_parallel_group())
    return _INNER_SEQUENCE_PARALLEL_RANK
...
def get_sequence_parallel_world_size():
    ...
    if not dist.is_initialized() or (_SEQUENCE_PARALLEL_GROUP is None):
        ...
        _SEQUENCE_PARALLEL_WORLD_SIZE = dist.get_world_size(group=get_sequence_parallel_group())
    return _SEQUENCE_PARALLEL_WORLD_SIZE
...
def setup_distributed(args: argparse.Namespace, kwargs: Optional[Dict] = None) -> Tuple[int, int, int]:
    """Setup distributed training environment."""
    if args.launcher != "none":
        set_multi_processing(distributed=True)
        init_dist(args.launcher, **kwargs)
