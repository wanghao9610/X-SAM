...
def parse_model_states(files, dtype=DEFAULT_DTYPE):
    ...
    for file in files:
        ...
        buffers = {k: v.to(dtype) for k, v in state_dict["module"].items() if k in buffer_names}
...
def _get_state_dict_from_zero_checkpoint(ds_checkpoint_dir, exclude_frozen_parameters, dtype=DEFAULT_DTYPE):
    ...
    zero_stage, world_size, flat_groups = parse_optim_states(optim_files, ds_checkpoint_dir, dtype)
    print(f"Detected checkpoint of type zero stage {zero_stage}, " f"world_size: {world_size}")

    model_files = get_model_state_files(ds_checkpoint_dir)

    zero_model_states = parse_model_states(model_files)
    print(f"Parsing checkpoint created by deepspeed==" f"{zero_model_states[0].ds_version}")
...
def _zero2_merge_frozen_params(state_dict, zero_model_states):
    ...
    if debug:
        ...
        print(f"Frozen params: Need {wanted_numel} numels in " f"{wanted_params} params")
    ...
    for name, shape in frozen_param_shapes.items():
        ...
        if debug:
            print(f"{name} full shape: {shape} unpartitioned numel " f"{unpartitioned_numel} ")

    print(f"Reconstructed Frozen state dict with {total_params} params " f"{total_numel} elements")
...
def _zero2_merge_trainable_params(state_dict, world_size, flat_groups, zero_model_states):
    ...
    avail_numel = sum([full_single_vector.numel() for full_single_vector in merged_single_partition_of_groups])
    ...
    total_numel = 0
    total_params = 0
    for shapes, full_single_vector in zip(param_shapes, merged_single_partition_of_groups):
        offset = 0
        ...
        for name, shape in shapes.items():
            ...
            if debug:
                print(f"{name} full shape: {shape} unpartitioned numel " f"{unpartitioned_numel} ")
            state_dict[name] = full_single_vector.narrow(0, offset, unpartitioned_numel).view(shape)
    ...
    print(f"Reconstructed state dict with {total_params} params " f"{total_numel} elements")


def _get_state_dict_from_zero2_checkpoint(world_size, flat_groups, zero_model_states, exclude_frozen_parameters):
    ...
    _zero2_merge_trainable_params(state_dict, world_size, flat_groups, zero_model_states)
...
def _zero3_merge_frozen_params(state_dict, world_size, zero_model_states):
    ...
    for name, shape in zero_model_states[0].frozen_param_shapes.items():
        ...
        state_dict[name] = torch.cat(param_frags, 0).narrow(0, 0, unpartitioned_numel).view(shape)  # noqa: E501
...
def _zero3_merge_trainable_params(state_dict, world_size, flat_groups, zero_model_states):
    param_shapes = zero_model_states[0].param_shapes
    ...
    if debug:
        ...
        print(f"Trainable params: Need {wanted_numel} numels in " f"{wanted_params} params.")
    ...
    print(f"Reconstructed Trainable state dict with {total_params} params " f"{total_numel} elements")


def _get_state_dict_from_zero3_checkpoint(world_size, flat_groups, zero_model_states, exclude_frozen_parameters):
    state_dict = OrderedDict()
    ...
    _zero3_merge_trainable_params(state_dict, world_size, flat_groups, zero_model_states)
...
def get_state_dict_from_zero_checkpoint(
    ...
    A typical usage might be ::
        ...
        from xsam.utils.dszero import get_state_dict_from_zero_checkpoint
    ...
    return _get_state_dict_from_zero_checkpoint(ds_checkpoint_dir, exclude_frozen_parameters, dtype)
...
def convert_zero_checkpoint_to_state_dict(
    ...
    state_dict = get_state_dict_from_zero_checkpoint(checkpoint_dir, tag, exclude_frozen_parameters, dtype)
    print(f"Saving {dtype} state dict to {output_file}")
    torch.save(state_dict, output_file)


def load_state_dict_from_zero_checkpoint(model, checkpoint_dir, tag=None, dtype=DEFAULT_DTYPE):
    # flake8: noqa
    """
    ...
    A typical usage might be ::
        ...
        from xsam.utils.dszero import load_state_dict_from_zero_checkpoint
