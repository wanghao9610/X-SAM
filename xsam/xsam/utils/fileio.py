...
def patch_fileio(global_vars=None):
    ...
    def glob(pathname, *, recursive=False):
        ...
        if pathname.endswith("*_optim_states.pt"):
            ...
            files = [os.path.join(pathname, f) for f in files if f.endswith("_optim_states.pt")]
        elif pathname.endswith("*_model_states.pt"):
            import os

            pathname = os.path.split(pathname)[0]
            files = backend.list_dir_or_file(pathname, recursive=recursive)
            files = [os.path.join(pathname, f) for f in files if f.endswith("_model_states.pt")]
        elif "*" in pathname:
    ...
    def copy(src, dst, **kwargs):
        ...
        elif isinstance(src_backend, LocalBackend) and isinstance(dst_backend, PetrelBackend):
...
def patch_hf_auto_from_pretrained(petrel_hub):
    ...
    target_cls.extend([PreTrainedTokenizerBase] + PreTrainedTokenizerBase.__subclasses__())
