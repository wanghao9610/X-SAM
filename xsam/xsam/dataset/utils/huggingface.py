...
from xsam.structures.pack import Packer
...
def build_origin_dataset(dataset, split):
    ...
    elif isinstance(dataset, dict) or isinstance(dataset, Config) or isinstance(dataset, ConfigDict):
...
def add_template_to_dataset(dataset, template_map_fn, map_num_proc):
    ...
    # remove invalid data
    dataset = dataset.filter(lambda example: len(example["conversation"]) > 0, num_proc=map_num_proc)
    return dataset
...
def tokenize_dataset(
    ...
    assert (tokenizer is not None) and (max_length is not None), f"({tokenizer}, {max_length})"
...
def pack_dataset(dataset, max_length, use_varlen_attn, shuffle_before_pack, map_num_proc):
...
def process(
    ...
    if use_varlen_attn:
        assert pack_to_max_length, (
            "`pack_to_max_length` in `process_hf_dataset` should be set to " "True if `use_varlen_attn` is True."
        )
    if pack_to_max_length:
        assert split == "train" or split is None, (
            "`split` should be `train` or `None` if `pack_to_max_length` is " f"True, but got {split}."
        )
    ...
    # pack to max length
    if pack_to_max_length:
        dataset = pack_dataset(dataset, max_length, use_varlen_attn, shuffle_before_pack, map_num_proc)
...
def process_hf_dataset(
    ...
    hf_dataset_timeout = timedelta(minutes=int(os.getenv("HF_DATASET_TIMEOUT", default=60)))
    print_log(f"hf_dataset_timeout = {hf_dataset_timeout}", logger="current")
    # monitored barrier requires gloo process group to perform host-side sync.
    group_gloo = dist.new_group(backend="gloo", timeout=hf_dataset_timeout)
    ...
    dist.monitored_barrier(group=group_gloo, timeout=hf_dataset_timeout)
