...
from xsam.model.modules.dispatch.parallel.sequence.attention import (
    post_process_for_sequence_parallel_attn,
    pre_process_for_sequence_parallel_attn,
)
from xsam.model.vlms.qwen3vl.modeling_qwen3_vl import apply_rotary_pos_emb, eager_attention_forward, repeat_kv
from xsam.utils.dist import get_sequence_parallel_world_size


# modified from xsam.model.vlms.qwen3vl.modeling_qwen3_vl.Qwen3VLTextAttention.forward
...
def qwen3vl_attn_forward(
    ...
    **kwargs: Unpack[FlashAttentionKwargs],
    ...
    # Qwen3VL applies q_norm and k_norm after q_proj and k_proj
    ...
    # different from Qwen3VLTextAttention.forward
    # repeat k/v heads if n_kv_heads < n_heads for sequence parallel
    key_states = repeat_kv(key_states, self.num_key_value_groups)
    value_states = repeat_kv(value_states, self.num_key_value_groups)

    enable_sequence_parallel = dist.is_initialized() and get_sequence_parallel_world_size() > 1 and self.training
    if enable_sequence_parallel:
        # Reshape for `pre_process_for_sequence_parallel_attn`
