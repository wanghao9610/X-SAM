...
from .comm import all_to_all, gather_forward_split_backward, split_forward_gather_backward


def pre_process_for_sequence_parallel_attn(query_states, key_states, value_states, scatter_dim=2, gather_dim=1):
    ...
    def pre_process_for_inner_sp(q, k, v):
        ...
        q = q.view(b, s_div_sp, sp // insp, h * insp // sp, insp, d // insp).transpose(3, 4).flatten(2, 4)
        k = k.view(b, s_div_sp, sp // insp, h * insp // sp, insp, d // insp).transpose(3, 4).flatten(2, 4)
        v = v.view(b, s_div_sp, sp // insp, h * insp // sp, insp, d // insp).transpose(3, 4).flatten(2, 4)
    ...
    if insp > 1:
        query_states, key_states, value_states = pre_process_for_inner_sp(query_states, key_states, value_states)
    ...
    if insp > 1:
        query_states, key_states, value_states = post_process_for_inner_sp(query_states, key_states, value_states)
...
def post_process_for_sequence_parallel_attn(attn_output, scatter_dim=1, gather_dim=2):
    ...
    if insp > 1:
        ...
        attn_output = split_forward_gather_backward(attn_output, -1, get_inner_sequence_parallel_group())
...
def sequence_parallel_wrapper(local_attn):
    def sequence_parallel_attn(query_states, key_states, value_states, *args, **kwargs):
        ...
        enable_sequence_parallel = dist.is_initialized() and get_sequence_parallel_world_size() > 1 and training
