...
class LLaVAModel(BaseModel):
    def __init__(
        ...
        **kwargs,
        ...
        self.visual_projector = ProjectorModel(projector_config).to(self.visual_encoder.dtype)
    ...
    def forward(self, data_dict, data_samples=None, mode="loss", **kwargs):
        if self.is_first_iter:
            ...
            self.to(data_dict["input_ids"].device)
    ...
    def _forward(self, data_dict, data_samples=None):
        if data_dict.get("inputs_embeds", None) is not None:
            data_dict["input_ids"] = None

        outputs = self.llm(**data_dict)
    ...
    def compute_loss(self, data_dict, data_samples=None):
        if data_dict.get("inputs_embeds", None) is not None:
            data_dict["input_ids"] = None
    ...
    def to_xtuner_llava(self, cfg, save_dir, fp32=False, save_pretrained_kwargs={}):
        ...
        # Projector
        projector_path = osp.join(save_dir, "visual_projector")
        print_log(f"Saving visual_projector to {projector_path}", "current")
        self.visual_projector.save_pretrained(projector_path, **save_pretrained_kwargs)

    def to_huggingface_llava(self, cfg, save_dir, fp32=False, save_pretrained_kwargs={}):
        LLM_MAPPING = {
        ...
        assert isinstance(llm, LlamaForCausalLM), "This conversion format only supports LlamaForCausalLM."
        llm_state_dict = llm.state_dict()
        llm_state_dict = convert_state_dict_to_hf(llm_state_dict, LLM_MAPPING)

        need_visual_encoder = not self.freeze_visual_encoder or self.use_visual_encoder_lora
        visual_encoder = self.visual_encoder
        if self.use_visual_encoder_lora:
            visual_encoder = self.visual_encoder.merge_and_unload()
        assert isinstance(visual_encoder, CLIPVisionModel), "This conversion format only supports CLIPVisionModel."
        if need_visual_encoder:
            visual_encoder_state_dict = visual_encoder.state_dict()
            visual_encoder_state_dict = convert_state_dict_to_hf(visual_encoder_state_dict, VIT_MAPPING)
        else:
            visual_encoder_state_dict = {}

        visual_projector_state_dict = self.visual_projector.state_dict()
        visual_projector_state_dict = convert_state_dict_to_hf(visual_projector_state_dict, PROJECTOR_MAPPING)

        state_dict = {
            **visual_projector_state_dict,
            **llm_state_dict,
            **visual_encoder_state_dict,
        }
        ...
        with init_empty_weights():
            with warnings.catch_warnings():
                warnings.filterwarnings("ignore", message=".*non-meta.*", category=UserWarning)
        ...
        if added_token > 0:
            ...
            model.language_model.model.embed_tokens.weight.data[ori_vocab_size:] = torch.stack(
                tuple(
                    dist.sample()
                    for _ in range(model.language_model.model.embed_tokens.weight.data[ori_vocab_size:].shape[0])
                ),
                dim=0,
            )
            model.language_model.lm_head.weight.data[ori_vocab_size:] = torch.stack(
                tuple(
                    dist.sample() for _ in range(model.language_model.lm_head.weight.data[ori_vocab_size:].shape[0])
                ),
                dim=0,
            )
